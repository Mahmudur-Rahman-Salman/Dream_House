...
import { facilities } from "@/constants/data";
import { useAppwrite } from "@/lib/useAppwrite";
import { fetchingAddPropertiesById } from "@/lib/appwrite";
      ...
      >
        ...
        {property?.facilities?.length > 0 && (
          <View className="px-5 mt-7">
            ...
            <View className="flex flex-row flex-wrap mt-2 gap-5">
              {property?.facilities.map((item: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | null | undefined, index: Key | null | undefined) => (
                <View
                  key={index}
        ...
        <View className="absolute bg-white bottom-0 w-full rounded-t-2xl border-t border-primary-200 p-7">
          <View className="flex flex-row items-center justify-between">
            ...
            <TouchableOpacity className="bg-primary-300 py-3 px-6 rounded-full shadow-md">
